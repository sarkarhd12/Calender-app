...
public class EventMetaDataController {
    ...
    public EventMetadata createEventMetaData(@RequestBody EventMetadata eventMetadata, HttpServletRequest request){
        String clientIp = request.getHeader("X-Forwarded-For");
        if (clientIp == null || clientIp.isEmpty()) {
            clientIp = request.getRemoteAddr();
        }

        // Set IP address in metadata
        eventMetadata.setIpAddress(clientIp);
